...
def goal_tracker(goal_amount_and_duration: str) -> str:
    ...
    try:
        ...
        # Get user data
        expenses_res = requests.get(f"{BASE_URL}/expenses/{user_id}/", headers=headers, timeout=10)
        ...
        incomes_res = requests.get(f"{BASE_URL}/incomes/{user_id}/", headers=headers, timeout=10)
